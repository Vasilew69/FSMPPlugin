package FSMP.FSMP.Votifier.net;
...
import FSMP.FSMP.Votifier.crypto.RSA;
...
public class VoteReceiver extends Thread {
     ...
     *            The port to listen on
     */
    public VoteReceiver(final FSMP plugin, String host, int port)
            throws Exception {
        this.plugin = plugin;
        this.host = host;
        this.port = port;
    ...
    public void run() {
        ...
        while (running) {
            try {
                ...
                writer.write("VOTY " + FSMP.getInstance().getVersion());
                writer.newLine();
                writer.flush();
                ...
                // Decrypt the block.
                block = RSA.decrypt(block, FSMP.getInstance().getKeyPair()
                        .getPrivate());
                int position = 0;
                ...
                for (VoteListener listener : FSMP.getInstance()
                        .getListeners()) {
                    try {
